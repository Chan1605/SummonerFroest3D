# 🌳 SummonerFroest 

> Unity 기반의 3D 개인포트폴리오 입니다.

![Unity](https://img.shields.io/badge/Unity-20232A?style=for-the-badge&logo=unity&logoColor=white)
![C#](https://img.shields.io/badge/-C%23-239120?style=for-the-badge&logo=csharp&logoColor=white)
![Status](https://img.shields.io/badge/status-Complete-brightgreen?style=for-the-badge)

## 🎬 Preview

> 
---

## 🧩 Game Features

- 🧠 AI 기반 몬스터 FSM 구현 (Trace / Attack / Die)
- 🎯 마우스 휠 줌아웃,피킹 이동, 근접 공격 | 플레이어 4가지 스킬 구현
- 🎮 플레이어 -> 믹사모를 사용한 오토리깅 후 애니메이터 적용
- 🎁 적 처치 시 아이템 드랍
- 🛠 스킬 아이콘 마우스 커서 근접 시 스킬설명 UI 추가
- 📦 오브젝트 풀링 기반 몬스터 리스폰 시스템
- 🪂 타이틀 / 인게임 / 게임오버 / 페이드 인 아웃 화면 완비

---

## 🗂 프로젝트 구조 요약

| 항목 | 설명 |
|------|------|
| 🎯 **플레이어** | 마우스 이동, 스킬, 버프, 아이템 획득,스킬 이펙트 |
| 👾 **적 AI** | FSM 기반 상태머신 (Idle/Trace/Attack/Die) |
| 📊 **UI 시스템** | 스킬 아이콘, 스킬 쿨타임, 버프 상태, 플레이어 체력바|
| 🧪 **테스트 및 개선** | 이슈 기반 QA 완료 및 최종 수정 반영 |

---

## ✅ 개발 로그 (주요 작업 내역)

- ✔️ 카메라 기준 캐릭터 회전 + 조준 확대
- ✔️ 오브젝트 풀링 최적화
- ✔️ FSM 구조 개선 및 디버그
- ✔️ 타이틀/로딩/게임오버 화면 연출
- ✔️ 피드백 기반 버그 수정 & 디테일 개선

---

## 🚀 다운로드

---

## 🧠 Tech Stack

- **Unity 2021.x 
- **C# / Visual Studio**
- **Git LFS 사용**
- **Shader Graph, Timeline 일부 활용**

---


## 🧪 피드백 반영

> ✅ 게임 전반 디버깅 및 QA 완료

---

## 🙋‍♂️ 개발자 정보

| 이름 | 링크 |
|------|------|
| 김종찬 (JongChan Kim) | [Github](https://github.com/Chan1605) / [Email](mailto:qkqhoe@naver.com) |

---

## 🔖 참고 문서

- [Unity 공식 매뉴얼](https://docs.unity3d.com/Manual/index.html)

---
